    ...
    def forward(self, input, max_len=20):
        ...
        if self.training:
            ...
            for med_code in [self.SOS_TOKEN] + input[2]:
                ...
                # print(attn_weight)
                input_embedding = attn_weight.mm(input_embedding) # (1, dim) #矩阵相乘
...
class Leap1(nn.Module):
    def __init__(self, voc_size, emb_dim=64, device=torch.device('cpu:0')):
        super(Leap1, self).__init__()
        self.voc_size = voc_size
        self.device = device
        self.SOS_TOKEN = voc_size[2]
        self.END_TOKEN = voc_size[2]+1
        ...
        self.dec_gru = nn.GRU(voc_size[3]+ emb_dim*2, emb_dim, batch_first=True)
    ...
    def forward(self, input, max_len=20):
        ...
        # print(input[0])
        input_tensor = torch.LongTensor(input[0]).to(device)
        feature_tensor = torch.tensor(input[3]).unsqueeze(dim=0).to(device)
        ...
        if self.training:
            ...
            for med_code in [self.SOS_TOKEN] + input[2]:
                ...
                _, hidden_state = self.dec_gru(torch.cat([feature_tensor,input_embedding, dec_input], dim=-1).unsqueeze(dim=0), hidden_state.unsqueeze(dim=0))
                # print(_)
                hidden_state = hidden_state.squeeze(dim=0) # (1,dim)
                # print(hidden_state)
                output_logits.append(self.output(F.relu(hidden_state)))
            ...
            for di in range(max_len):
                ...
                attn_weight = F.softmax(self.attn(combined_input).t(), dim=-1)  # (1, len)
                input_embedding = attn_weight.mm(input_embedding)  # (1, dim)
                _, hidden_state = self.dec_gru(torch.cat([feature_tensor,input_embedding, dec_input], dim=-1).unsqueeze(dim=0),
                                               hidden_state.unsqueeze(dim=0))
                hidden_state = hidden_state.squeeze(dim=0)  # (1,dim)
                output = self.output(F.relu(hidden_state))
                topv, topi = output.data.topk(1)
